  ...
  getInfosBtcAddress(){
    this.niceHashService.getInfosFromBtcAddress().subscribe({
      next: value => {
        ...
        console.log("Next : ", new Date(this.results.nextPayoutTimestamp).getTimezoneOffset())
        var now = new Date().getTime()
        var remaining = new Date(next - now).getMinutes()
        var hoursRemaining = (remaining/60).toFixed(0)
        this.nextPayout.next(hoursRemaining+"h"+remaining+"m")
